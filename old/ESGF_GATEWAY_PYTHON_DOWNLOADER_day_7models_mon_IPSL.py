...
# variable names
models=["IPSL-CM5A-LR"] # "GISS-E2-R","GFDL-CM3","CNRM-CM5","MPI-ESM-LR","CCSM4", "MRI-CGCM3" # if all models want to be accessed simply comment this line out
experiments = ["historical","rcp45","rcp60","rcp85"]#,"rcp26",
time_frequencies = ["mon"]
realms = ["atmos"] 
CMOR_tables = ["Amon"] 
ensembles = ["r1i1p1","r2i1p1"] 
variables = ["tas","tasmin","tasmax","pr","psl","uas","vas","hur","hurs","hus","huss","rsds","clt"]#,"sit","sic" "sfcWind","tauu","tauv","ua","va"
...
outPath="/scratch/malindgren/CMIP5/wget"
downPath="/scratch/malindgren/CMIP5/data"
...
if os.path.exists(os.path.join(outPath,'wgetLogFile_'+dateTime+'.txt')) == False:
	...
	outFile=open(os.path.join(outPath,'wgetLogFile_'+dateTime+'.txt'),'a')
#open a csv writer
f=open(os.path.join("/scratch/malindgren/CMIP5/other","wget_masterTable_mon.csv"),'w')
c = csv.writer(f)
lineList=[]
						...
						for ensemble in ensembles:
							for variable in variables:
								...
								try:
									...
									wget_file=open(os.path.join(outPath,model+'_'+variable+'_'+experiment+"_"+time_frequency+"_"+realm+'_'+CMOR_table+'_'+ensemble+'.sh'), "rt")
...
##		os.system('wget '+wget_command_credentials+" "+line.split()[1])
...
# # not sure if I really want this stupid part in the code
