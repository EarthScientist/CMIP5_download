...
# variable names
models=["GISS-E2-R","GFDL-CM3","CNRM-CM5","IPSL-CM5A-LR","MPI-ESM-LR","CCSM4", "MRI-CGCM3"] # if all models want to be accessed simply comment this line out
experiments = ["rcp60"]#,"rcp26",
time_frequencies = ["mon"]
realms = ["atmos"] 
CMOR_tables = ["Amon"] 
ensembles = ["r1i1p1"] 
variables = ["tas","pr","psl"] #,"sit","sic"
...
outPath="/big_storage/malindgren/wget/wget_rcp60/"
downPath="/big_storage/malindgren/data/"
...
if os.path.exists(os.path.join(outPath,'wgetLogFile_'+dateTime+'.txt')) == False:
	...
	outFile=open(os.path.join(outPath,'wgetLogFile_'+dateTime+'.txt'),'a')
#open a csv writer
f=open(os.path.join("/big_storage/malindgren/","wget_masterTable_mon.csv"),'w')
c = csv.writer(f)
lineList=[]
