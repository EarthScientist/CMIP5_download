...
variables = ["tas","pr","psl","uas","vas","sfcWind","tauu","tauv","ua","va"]
...
if os.path.exists(os.path.join(outPath,'wgetLogFile_'+dateTime+'.txt')) == False:
	...
	outFile=open(os.path.join(outPath,'wgetLogFile_'+dateTime+'.txt'),'a')
#open a csv writer
f=open(os.path.join("/scratch/malindgren/CMIP5/other","wget_masterTable_day.csv"),'w')
c = csv.writer(f)
lineList=[]
...
try:
	f=open(os.path.join("/scratch/malindgren/CMIP5/other","wget_masterTable_day.csv"),'r')
